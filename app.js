...
app.use((req, res, next) => {
  req.user = {
    _id: "634848dd7c302d840461792b", // Id взято из бд "mestodb" в папке users, это id тестового юзера
  };
...
app.all("*", (req, res) => {
  res.status(404).send({ message: "Неправильный путь" }); // На все запросы, кроме двух верхних сервер пока будет отвечать "Неправильный путь"
});
